...
void vu() {
  ...
  // Draw peak dot  
  if(peak > 0 && peak <= NUM_LEDS-1) leds[peak] = CRGB(0,255,0);
...
void calculatePeakAndHeight() {
  ...
  if(height < 0L)       height = 0;      // Height cant be less than 0 (0L just means (long)0)
  else if(height > TOP) height = TOP;    // Height cant be greater than TOP
